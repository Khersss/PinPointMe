...
class _PendingDashboardState extends State<PendingDashboard> {
  int _selectedBottomNavIndex = 0; // Default index is Home
  ...
  List<Widget> _buildContent(BuildContext context) {
    return [
      ...
      BottomNavigationBar(
        ...
        onTap: (btmIndex) {
          setState(() => _selectedBottomNavIndex = btmIndex);
          ...
          switch (btmIndex) {
            ...
            case 1: // Inbox
              Navigator.push(context, MaterialPageRoute(builder: (context) => RescuerInbox())).then((_) {
                // Reset bottom nav when returning from RescuerInbox
                setState(() {
                  _selectedBottomNavIndex = 0; // Set back to home
                });
              });
              break;
            case 2: // Profile
              Navigator.push(context, MaterialPageRoute(builder: (context) => RescuerProfile())).then((_) {
                // Reset bottom nav when returning from RescuerProfile
                setState(() {
                  _selectedBottomNavIndex = 0; // Set back to home
                });
              });
